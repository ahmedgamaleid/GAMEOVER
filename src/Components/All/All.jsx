  ...
  return (
    <>
        {/* <div className="col-md-3">
          <div className="box shadow rounded-2">
        <img src={getgames.thumbnail} alt="" className='w-100'/>
        <div className="d-flex justify-content-between align-items-center p-2">
        <h3 className='fs-6 my-2'>{getgames.title}</h3>
        <span className='btn btn-info text-white'>Free</span>
        </div>
        <p className='w-100 overflow-hidden fs-6 mb-2 p-2'>{getgames.short_description}</p>
        ...
        <div>
          <span className={`p-1 mx-1 rounded-3 ${styles.spanbg}`}>{getgames.genre}</span>
          <span className={`p-1 rounded-3 ${styles.spanbg}`}>{getgames.platform}</span>
        </div>
        </div>
          </div>
        </div> */}
